...
MqttModel::MqttModel(QObject *parent)
    : QAbstractTableModel(parent), _timer(new QTimer(this)) {
    _qthread = new QThread();
    this->moveToThread(_qthread);
    _timer->moveToThread(_qthread);
        _qthread->start();
  _timer->setInterval(1000);
  connect(_timer, &QTimer::timeout, this, &MqttModel::timerHit);
  _timer->start();
}
...
void MqttModel::timerHit() {
  ...
  QModelIndex topLeft = createIndex(1, 1);
   Q_EMIT dataChanged(topLeft, topLeft, {Qt::DisplayRole});
}
...
void MqttModel::update(std::string topic, std::string message) {
  ...
  for (uint32_t i = 0; i < table.size(); i++) {
    if (table[i].topic == topic) {
      ...
      table[i] = newEntry;
     Q_EMIT  dataChanged(left, right, {Qt::DisplayRole});
    }
  }
  if (!found) {
...
void MqttModel::sort(int ) { std::sort(table.begin(), table.end()); }
